...
public class MainClass extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == bn1) {
			...
			if (select1 == JOptionPane.CLOSED_OPTION) {
				...
				test2.mapListAdd(id, nam, phone, gen);
				clear();
	...
	/*	 Enumeration<AbstractButton> enums = bg.getElements();
		...
		if (e.getSource() == bn3) {
			...
			ArrayList<MemberData> list = test2.getMapList();
			
			ta.setText("회원ID" + "\t" + "회원이름" + "\t" + "전화번호" + "\t" + "성별" + "\n");
			for (int i = 0; i <= list.size(); i++) {
			
				str +=  list.get(i).getMemberID() + list.get(i).getName() + list.get(i).getTel()+ list.get(i).getGender()+"\n";
			}
			ta.append(str);
		}
